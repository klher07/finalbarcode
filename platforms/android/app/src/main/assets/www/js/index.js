...
function validarBoletocodigo(e){
    console.log(e.keyCode);
    //var sk=$('#onlycodigo').val();
       //var porciones = sk.split(' ');
       //var porci=porciones[0];
    if(e.keyCode == 13){
        ...
        var db = window.openDatabase("Database", "1.0", "TicketMobile", 200000);
        db.transaction(function(tx){
            tx.executeSql('SELECT * FROM Boleto WHERE strBarcode = ? limit 1;',[codigo],function(tx,results){
                console.log(results);
...
function validarIngresocodigo3(){
        var codigo = $('#onlycodigo').val();
        var db = window.openDatabase("Database", "1.0", "TicketMobile", 200000);
        db.transaction(function(tx){
